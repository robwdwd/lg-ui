<template>
  <UContainer>
    <div class="flex flex-col lg:flex-row gap-5">
      <div>
        <MultiForm :max-destination="$config.public.maxDestPing" :max-location="$config.public.maxLocsPing" class="space-y-4" @submit="onSubmit" />
      </div>
        ...
        <div v-if="results">
          <UTabs :items="tabItems" class="w-full">
            <template v-for="location in results.locations" #[location.name]="{ item }">
              <PingResult :results="location.results" />
            </template>
          </UTabs>
        </div>
      </div>
...
const results = ref<MultiPingResult | null>(null);

async function onSubmit(event: MultiEmitSchema) {
  results.value = null;
  tabItems.value = [];

  const response = await fetchResults<MultiPingResult>('/api/multi/', {
    command: 'ping',
    body: event,
